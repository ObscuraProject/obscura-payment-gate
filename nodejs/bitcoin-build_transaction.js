...
function buildTransaction(utxosets, outs, fee) {
  ...
  for (var i = 0; i < utxosets.length; i++) {
    for (var j = 0; j < utxosets[i].utxo.length; j++) {
      // console.log(utxosets[i].utxo[j].txid, utxosets[i].utxo[j])
...
function explainFee(utxosets, rawTransaction, feePerByte) {
  ...
  return (
    "Number of unspent inputs: " +
    utxoLength +
    "; Transaction size (bytes): " +
    hexToBytes(rawTransaction.toHex()).length +
    "; Fee per byte: " +
    feePerByte
  );
}
...
var utxosets = JSON.parse(
  new Buffer.from(argv.utxos, "base64").toString("ascii")
);
...
if (feeOnly) {
  ...
  if (fee >= outs[feeOutput].satoshis) {
    result.error =
      "Fee is greater than tx rem:" +
      fee +
      "; output: " +
      outs[feeOutput].satoshis;
  } else {
